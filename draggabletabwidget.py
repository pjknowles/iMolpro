...
if False:
    ...
    class DraggableTabBar(QtWidgets.QTabBar):
        ...
        def mousePressEvent(self, event):
            ...
            if event.button() == QtCore.Qt.MouseButton.LeftButton:
                current_index = self.tabAt(event.position().toPoint())
                parent = self.parent()
                ...
                self.click_point = event.position()
                self.can_start_drag = False
                self.grabMouse()
        ...
        def mouseMoveEvent(self, event):
            ...
            if not self.can_start_drag:
                moved_length = (event.position() - self.click_point).manhattanLength()
            ...
            if cls.dragging_widget_:
                for bar_inst in cls._tabBarInstances():
                    ...
                    if bar_region.contains(event.globalPosition()):
                        if (bar_inst == self):
                            self.startTabMove()
                            event.accept()
                            return
            ...
            if widget_rect.contains(event.position().toPoint()):
        ...
        def startDrag(self):
            ...
            release_event = self.createMouseEvent(
                ...
                self.mapFromGlobal(QtGui.QCursor.position()))
            QtWidgets.QApplication.postEvent(self, release_event)

        def createMouseEvent(self, event_type, pos = QtCore.QPointF()):
            if not isinstance(pos, QtCore.QPointF):
                pos = QtCore.QPointF(pos)
            if pos.isNull():
                global_pos = QtGui.QCursor.position()
            else:
    ...
    # Custom TabBar to enable dragging
    class DraggableTabBar(QTabBar):
        def __init__(self, parent=None):
            super().__init__(parent)
            self.setMovable(True) # Core functionality for drag-and-drop

    class DraggableTabWidget(QTabWidget):
        def __init__(self, parent=None):
            super().__init__(parent)
            self.setTabBar(DraggableTabBar(self)) # Apply custom bar
            self.setTabsClosable(True) # Adds close button
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PyQt6 Draggable Tabs")
        self.resize(600, 400)
        ...
        # Add sample tabs
        for i in range(1, 4):
            self.tab_widget.addTab(QLabel(f"Content {i}"), f"Tab {i}")
